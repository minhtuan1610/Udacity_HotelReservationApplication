		...
		while (choice != 0) {
			...
			switch (choice) {
				case 1 -> {
					...
					Collection<IRoom> roomAlterFound = roomFound.isEmpty() ? HOTEL_RESOURCE.findAlternativeRoom(checkIn, checkOut) : new ArrayList<>();
					...
					if (!roomFound.isEmpty() || !roomAlterFound.isEmpty()) {
						...
						if (text.equalsIgnoreCase("y")) {
							try {
								...
								if (roomFound.contains(HOTEL_RESOURCE.getRoom(rNumber))
										|| roomAlterFound.contains(HOTEL_RESOURCE.getRoom(rNumber))) {
